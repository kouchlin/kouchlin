...
class CouchDatabaseDocument(val db: CouchDatabase, val id: String? = null, val rev: String? = null) {
	...
	fun exists(etag: String? = null): Triple<Int?, String?, STATUS> {
		...
		val (_, response, _) = documentURI.httpHead().configureAuthentication(db.server).header(headers).response()
	...
	fun get(attachment: Boolean? = null,
			...
			etag: String? = null): Triple<String?, String?, STATUS> {
		...
		val (request, response, result) = documentURI.httpGet(parameters).configureAuthentication(db.server).header(headers).responseString()
